...
Semaphore：信号量，用来设置最大并发量，数据库连接池就可以使用Semaphore来实现。
           ...
           被阻塞的线程任务都被放入到队列中，等待执行中的线程执行完毕，然后调用Semaphore.release()释放信号量，
           然后唤醒队列中的线程去执行。
           推荐使用 tryAcquire和tryRelease
Exchanger: 两个线程执行到某个点时，交换数据。Exchanger.exchange(T object)表示线程会在这里阻塞，等待另一个线程也执行到Exchanger.exchange(T Object)时，交换数据、
           数据交换完成后继续执行。
